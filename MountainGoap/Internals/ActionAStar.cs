        ...
        private static float Heuristic(ActionNode actionNode, BaseGoal goal, ActionNode current) {
            ...
            if (goal is Goal normalGoal) {
                ...
                foreach (var kvp in extremeGoal.DesiredState) {
                    ...
                    else if (kvp.Value && actionNode.State[kvp.Key] is object a && current.State[kvp.Key] is object b && IsHigherThanOrEquals(a, b)) cost += 1f / valueDiff;
                    else if (!kvp.Value && actionNode.State[kvp.Key] is object a2 && current.State[kvp.Key] is object b2 && IsLowerThanOrEquals(a2, b2)) cost += 1f / (1 - valueDiff);
                }
            }
